...
void ticketing(int seat_rmg[3][3], int seat_num[3][3], int seat_sold[3][3],
	int c[3], int &m, Operation &all_counter, Operation counter[3][3], Operation movie_counter[3]);
...
int main()
{
	...
	int seat_num[3][3] = { {0,0,0},
							{0,0,0},
							{0,0,0}, }; //seat_num[movie][class]
	...
	int c[3] = { 0,0,0 }; //Determine if the counter for this movie closes, c[movie]
	...
	if (access == true)
	{
		do
		{
			...
			case 2:
				...
				break;
			case 3:
				ticketing(seat_rmg, seat_num, seat_sold, c, m, all_counter, counter, movie_counter);
...
{
	//Declaration of Variables
	int movie, display[3], seat_choice; //Display movie seats available, display[class], [0] = class 0, [1] = class 1, [2] = class 2
	...
	movie--; //To prevent confusion for programmer, sync to become 0,1,2
	...
	int c[3], int &m, Operation &all_counter, Operation counter[3][3], Operation movie_counter[3])
{
	...
	while (all_counter == OPEN && cont == NEXT_CUSTOMER)
	{
		...
		total = 0; //Clear total accumulated
		for (int i = 0; i < 3; i++)
		{
		...
		for (int j = 0; j < 3; j++) //Prints out category for adult, then child, then student
		{
			for (count[j]; count[j] > 0; count[j]--) //Prints out number of tickets for each category
			{
				...
				if (seat_num[movie][tclass] < 10)
					cout << "  Seat        : " << tclass << "0" << seat_num[movie][tclass] << endl;
				else
					cout << "  Seat        : " << tclass << seat_num[movie][tclass] << endl;
		...
		for (int i = 0; i < 3; i++) //Check each movie
		{
			for (int j = 0; j < 3; j++) //Check each class in each movie
			{
				...
				{
					if (counter[i][j] != CLOSE) //Prevent repeated addition to counter c
						c[i]++; //One class unavailable for this movie
					counter[i][j] = CLOSE; //This counter closes
				}
				else
					counter[i][j] = OPEN;
			}
			if (c[i] == 3) //All counters for this movie are sold out
			{
				if (movie_counter[i] != CLOSE) //Prevent repeated addition to counter m
					m++;
	...
	if (all_counter == CLOSE)
		cout << "  All tickets for all movies have been sold out. Please come again next week!\n";
}
...
void report(const int MAX_SEAT[3][3], int seat_sold[3][3])
{
	...
	float cat_revenue[3][3] = { {0,0,0}, {0,0,0}, {0,0,0} }; //cat_revenue[category][class]
	float revenue[3] = { 0,0,0 }; //revenue[category]
	int all_class = 0, all_seat = 0, all_cat = 0;
	float all_revenue = 0;
